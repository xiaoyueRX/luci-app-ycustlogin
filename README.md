...
1. **断线自动重连**：每 30 秒探测一次，检测到掉线瞬间自动补登。
2. **状态仪表盘**：后台面板实时显示账号状态、当前在线设备数（如 2/2）以及其他占坑设备的 IP。
3. **强制重新登录**：由于网关底层接口不支持“点对点精准踢人”，本插件采用“注销再秒登”的策略。当设备数满载导致网络卡顿时，一键发送重连请求，通常网关会释放掉建立时间最长且非活跃的连接，从而变相将其他伪装或挂机设备“挤下线”。

## 工作原理

每 30 秒向 Dr.COM 认证服务器发送底层探测请求：

- `error5` → 已在线，静默不打扰
- `result:1` → 刚刚掉线，现已成功补登
- `result:0` → 认证失败（可能密码被改或欠费）
- 超时/无响应 → Portal 彻底不可达，记录警告
